<template>
    <div id="CreateGoods">
        ...
        <nav @click="address">
            ...
            <div v-if="listadress.length > 0" class="nav_address">
                <p>收货人：{{defaultAdress.ship_name}}</p><span>{{defaultAdress.phone_num}}</span>
                <div>收货地址：{{defaultAdress.province+defaultAdress.city+defaultAdress.area+defaultAdress.address}}</div>
            </div>
            <img src="../../assets/img/yishoujiantou.png" alt="">
        </nav>

        <div v-if="CategoryTwoOrThreeListDetail" class="CreateGoods_Commodity">
            <h4>商品信息</h4>
            <div class="CreateGoods_Commodity_">
                <div class="CreateGoods_Commodity_content">
                    ...
                    <section>
                        ...
                        <div>
                            <span>{{CategoryTwoOrThreeListDetail.price}}元</span>
                            <p><van-stepper v-model="value" /></p>
                        </div>
                    </section>
                </div>
                <!-- <div class="CreateGoods_Commodity_time"><p>有效期</p><span>永久有效</span></div>
                <div class="CreateGoods_Commodity_time"><p>适用商家</p><span>全场通用</span></div> -->
            </div>
            <div class="CreateGoods_Commodity_nowPrice"><span>小计：</span><span>{{(CategoryTwoOrThreeListDetail.price*value).toFixed(2)}}元</span></div>
        </div>

        <div class="CreateGoods_footer">
            <div><span>合计：</span><small>¥ </small><span>{{(CategoryTwoOrThreeListDetail.price*value).toFixed(2)}}</span></div>
        ...
        <van-actionsheet class="actionsheet" v-model="show">
            ...
            <div @click="$router.push('/Address')" class="addressList_add">添加地址</div>
        </van-actionsheet>
...
export default {
    ...
    computed:{
        ...
        defaultAdress(){
            return this.$store.state.defaultAdress
        },
    ...
    methods:{
            ...
            let order = { 
                ...
                orderGoods:[ 
                    { 
                        ...
                        oldPrice: this.CategoryTwoOrThreeListDetail.market_price, thumbPic: this.CategoryTwoOrThreeListDetail.thumbnail_pic 
                    }
                ]
            }
        ...
        changeAddress(index){
            this.show = false
            this.$store.commit('defaultAdress', this.listadress[index])
        },
        getAddress(){
...
.CreateGoods_Commodity{
    width: 100%; height: 4.2rem; background-color: white; margin-top: 0.2rem;
    h4{ color:rgba(43,43,43,1); .font1; line-height: 1.1rem; margin-left: 0.4rem; }
    .CreateGoods_Commodity_{
        width: 100%; height: 2.2rem; padding: 0 0.4rem; border-top: 0.015rem solid rgba(206,206,206,1); border-bottom: 0.015rem solid rgba(206,206,206,1);
        .CreateGoods_Commodity_content{
        ...
        // }
        // .CreateGoods_Commodity_time:last-child{ border: 0!important; }
    }
    .CreateGoods_Commodity_nowPrice{
