<template>
    <div id="SecondKill">
        ...
        <!-- <div v-if="groupKillList.length > 0">
        ...
        <div class="goods_list" v-if="groupKillList.length > 0">

            <div class="goods_list_" v-for="(item,index) in groupKillList[tabNum].data" :key="index">
                <a :href="'http://www.homeamc.cn/#/Crab?&statusId='+item.id">
                    <div class="goods_list_content">
                        <section><img :src="imgUrl + item.path" alt=""></section>
                        <div class="goods_list_contents">
                            <div class="goods_list_contents_name">{{item.title}}</div>
...
export default {
    ...
    computed:{
        ...
        groupKillList(){
            // console.log(this.$store.state.groupKillList)
            return this.$store.state.groupKillList
        },
        killTimeList(){
            let list = []
            for(let val of this.groupKillList.data){
                list.push({day: `${val.start_time.substring(5,7)}月${val.start_time.substring(8,10)}日`, hour: val.start_time.substring(11,16), status: val.data[0].status})
            }
            return list
        },
        time(){
...
.SecondKill_time{
    ...
    .active_times{
        background: url('../../assets/home/active_times.png') no-repeat!important; background-size: 100% 100%!important;
    }
    .active_time_after{
        background: url('../../assets/home/active_time_after.png') no-repeat; background-size: 100% 100%;
    }
}
.Count_down{
