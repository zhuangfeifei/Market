<template>
    <div id="CouponAll">
        ...
        <div v-if="groupList.length > 0" class="CouponAll_">
            <div class="CouponAll_list_Cash" @click="groupListdetails(index)" v-for="(item,index) in groupList" :key="index">
                <van-row>
                    <van-col span="5" class="CouponAll_list_Cash_title"><img :src="imgUrlGoods + item.thumbnail_pic" alt=""></van-col>
                    <van-col span="19" class="CouponAll_list_Cash_ammont">
                        <p>{{item.group_name}}</p>
                        <div><del>原价：¥{{item.discount}}</del><span>¥{{item.price}}</span></div>
                    </van-col>
                </van-row>
            </div>
        </div>
...
export default {
    data() {
        return {
            ...
            list:{ limit:100, current:1, isPage: false, tabIndex: 0 }
        }
    },
    components: {
